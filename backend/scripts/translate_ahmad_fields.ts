/**
 * Ahmad Hadith Translation Script using DeepSeek API
 * Translates: kitab.th, bab.th, chain.th, content.th from Arabic to Thai
 */
...
สำหรับ bab (บท) - แปลความหมายเป็นภาษาไทย
สำหรับ chain (สายรายงาน) - ทับศัพท์ชื่อผู้รายงานเป็นภาษาไทย
สำหรับ content (เนื้อหา) - แปลความหมายเป็นภาษาไทย`
                },
                {
                    role: 'user',
                    content: prompt
...
async function translateBatch(limit: number = 10, offset: number = 0, mode: 'kitab' | 'chain' | 'bab' | 'content' | 'all' = 'all') {
    console.log(`\n📚 Starting Ahmad ${mode} translation`);
    ...
    try {
        ...
        if (hadiths.length === 0) {
            console.log('✨ All Ahmad hadiths already translated!');
            return;
        }
...
async function generateTranslation(hadith: Hadith, mode: string, retries: number = 2): Promise<TranslationResult | null> {
    ...
    const needsKitab = ['kitab', 'all'].includes(mode) && kitabAr && !hadith.kitab?.th;
    const needsBab = ['bab', 'all'].includes(mode) && babAr && !hadith.bab?.th;
    const needsChain = ['chain', 'all'].includes(mode) && chainAr && !hadith.chain?.th;
    const needsContent = ['content', 'all'].includes(mode) && contentAr && !hadith.content?.th;
    ...
    let prompt = `Translate this Hadith from Musnad Ahmad to Thai. Return ONLY raw JSON without markdown formatting.\n\n`;
...
คำแนะนำ:
1. "chain" - ถอดเสียงชื่อนักรายงานเป็นไทย เช่น "อับดุลลอฮฺ บิน อุมัร > อันนะสาอีย์" ใช้ " > " คั่นระหว่างนักรายงาน
...
if (args[0] === 'help' || args[0] === '--help') {
    console.log(`
📚 Ahmad Hadith Translation Script (DeepSeek)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Usage: npx tsx scripts/translate_ahmad_fields.ts [mode] [limit] [offset]
...
Examples:
  npx tsx scripts/translate_ahmad_fields.ts all 100        # Translate 100 ahmad hadiths
  npx tsx scripts/translate_ahmad_fields.ts content 50     # Translate content only
  npx tsx scripts/translate_ahmad_fields.ts chain 20 100   # Skip first 100, translate 20 chains
    `);
    process.exit(0);
}

...
# Ibn Majah JSON URL (سنن ابن ماجه)
JSON_URL = "https://raw.githubusercontent.com/fawazahmed0/hadith-api/refs/heads/1/editions/ara-ibnmajah.json"
...
def fetch_json():
    ...
    print(f"📥 Fetching Ibn Majah (سنن ابن ماجه) from {JSON_URL}...")
    try:
...
def import_ibnmajah():
    ...
    for h in hadiths:
        try:
            ...
            hadith_id = f"ibnmajah_{hadith_no}"
    ...
    print(f"\n=== 🎉 Import Complete (Ibn Majah) ===")
    print(f"✅ Imported: {count}")
    print(f"⏭️ Skipped (already exists): {skipped}")
    print(f"❌ Errors: {errors}")
    print(f"\n📊 Total Ibn Majah in DB: {translations_collection.count_documents({'hadith_book': 'ibnmajah'})}")

...
# Tirmidhi JSON URL (جامع الترمذي)
JSON_URL = "https://raw.githubusercontent.com/fawazahmed0/hadith-api/refs/heads/1/editions/ara-tirmidhi.json"
...
def fetch_json():
    ...
    print(f"📥 Fetching Tirmidhi (جامع الترمذي) from {JSON_URL}...")
    try:
...
def import_tirmidhi():
    ...
    for h in hadiths:
        try:
            ...
            hadith_id = f"tirmidhi_{hadith_no}"
    ...
    print(f"\n=== 🎉 Import Complete (Tirmidhi) ===")
    print(f"✅ Imported: {count}")
    print(f"⏭️ Skipped (already exists): {skipped}")
    print(f"❌ Errors: {errors}")
    print(f"\n📊 Total Tirmidhi in DB: {translations_collection.count_documents({'hadith_book': 'tirmidhi'})}")
